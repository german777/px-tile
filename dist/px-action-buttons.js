'use strict';(function(){Polymer({is:'px-action-buttons',properties:{/**
       ...
       */isPrimary:{type:Boolean,value:false}},created:function created(){this._maxIcons=3},/**
...
this._notifyActionChange=false;//this._isDisplayDropdown = this.actionButtons && this.actionButtons.items && this.actionButtons.items.length > this._maxIcons;
var actionBtns=JSON.parse(JSON.stringify(this.actionButtons));var dropdownCount=0;for(var x in actionBtns.items){if(!actionBtns.items[x].isPrimary){dropdownCount++}}this._isDisplayDropdown=dropdownCount>this._maxIcons;if(this.isPrimary){this._isDisplayButtons=true;this._isDisplayDropdown=false}else if(this._isDisplayDropdown){this.async(function(){var pxDropdown=this.$$('#pxDropdown');pxDropdown.style.height='20px';for(var _x in actionBtns.items){// px-dropdown has key and val
var item=actionBtns.items[_x];item.key=item.id||item.key;item.val=item.label||item.val;delete item.id;delete item.label;if(!actionBtns.multi){delete item.selected}}// there are only two options for sortMode and selectBy in px-dropdown
